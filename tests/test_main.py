...
import os
from hkopenai.hk_food_mcp_server.__main__ import cli_main
...
class TestMain(unittest.TestCase):
    ...
    @patch('hkopenai.hk_food_mcp_server.__main__.server')
    def test_default_behavior(self, mock_server, mock_arg_parser):
        """
        ...
        # Assertions
        mock_server.assert_called_once_with(host='127.0.0.1', port=8000, sse=False)
    ...
    @patch('argparse.ArgumentParser')
    @patch('hkopenai.hk_food_mcp_server.__main__.server')
    def test_command_line_arguments(self, mock_server, mock_arg_parser):
        """
        ...
        with patch.dict(os.environ, {'TRANSPORT_MODE': 'stdio', 'HOST': '0.0.0.0', 'PORT': '9000'}):
            ...
            # Assertions
            mock_server.assert_called_once_with(host='192.168.1.1', port=8080, sse=True)
            mock_parser_instance.parse_args.assert_called_once()
            mock_parser_instance.get_default.assert_any_call('host')
            mock_parser_instance.get_default.assert_any_call('port')
    ...
    @patch('argparse.ArgumentParser')
    @patch('hkopenai.hk_food_mcp_server.__main__.server')
    def test_environment_variables(self, mock_server, mock_arg_parser):
        """
        ...
        with patch.dict(os.environ, {'TRANSPORT_MODE': 'sse', 'HOST': '0.0.0.0', 'PORT': '9000'}):
            ...
            # Assertions
            mock_server.assert_called_once_with(host='0.0.0.0', port=9000, sse=True)
            mock_parser_instance.parse_args.assert_called_once()
            mock_parser_instance.get_default.assert_any_call('host')
            mock_parser_instance.get_default.assert_any_call('port')
    ...
    @patch('argparse.ArgumentParser')
    @patch('hkopenai.hk_food_mcp_server.__main__.server')
    def test_sse_mode_from_env(self, mock_server, mock_arg_parser):
        """
        ...
        with patch.dict(os.environ, {'TRANSPORT_MODE': 'sse'}):
            ...
            # Assertions
            mock_server.assert_called_once_with(host='127.0.0.1', port=8000, sse=True)
            mock_parser_instance.parse_args.assert_called_once()
            mock_parser_instance.get_default.assert_any_call('host')
            mock_parser_instance.get_default.assert_any_call('port')
    ...
    @patch('argparse.ArgumentParser')
    @patch('hkopenai.hk_food_mcp_server.__main__.server')
    def test_invalid_port_env_variable(self, mock_server, mock_arg_parser):
        """
